...
local persisted = settings.load(T{
    ...
    tracker_bg_alpha = 0.90,
    tracker_font_scale = 1.00,
});


-- Side tracker display preferences are persisted globally so players can tune
-- the overlay once and keep the same layout/readability between sessions.
...
persisted.tracker_bg_alpha = clamp(tonumber(persisted.tracker_bg_alpha) or 0.90, 0.00, 1.00);
persisted.tracker_font_scale = clamp(tonumber(persisted.tracker_font_scale) or 1.00, 0.75, 1.50);
...
local function character_is_in_game()
    local ok, ready = pcall(function()
        ...
        if name == nil or #tostring(name) == 0 then return false; end

        -- A valid local player entity/index prevents the addon windows from
        -- appearing on title / character-select screens where a stale name
        -- can occasionally still be present.
        local target = mm:GetTarget();
        if target ~= nil and target.GetIsSubTargetActive ~= nil then
            -- Merely touching the target manager here verifies the in-game
            -- memory managers are initialized; no target state is changed.
        end
        return true;
    end);
...
local state = {
    ...
    keyitem_overrides = {},
    pending_keyitem_chat_action = nil,
    pending_keyitem_chat_until = 0,
    zoning = false,
...
-- Only permanent / collection-style KIs are allowed to restore ownership
-- from disk. Pop key items are consumable, so a saved TRUE can become stale
-- while the addon is unloaded. They must be confirmed this session by an
-- authoritative 0x055 page, a live obtain/loss event, or a positive client
-- memory result.
local cached_persistent_ki_names = nil;
local function is_persistent_keyitem_name(name)
    if cached_persistent_ki_names == nil then
        cached_persistent_ki_names = {};
        for _, entry in ipairs(data.abyssites or {}) do
            cached_persistent_ki_names[normalize(entry.name)] = true;
        end
        for _, entry in ipairs(data.atmas or {}) do
            cached_persistent_ki_names[normalize(entry.name)] = true;
        end
    end
    return cached_persistent_ki_names[normalize(name)] == true;
end

local function keyitem_verified_this_session(name)
    if not state.lookup_ready then rebuild_lookups(); end
    local ids = resolve_keyitem_ids(name);
    if ids == nil or #ids == 0 then return false; end
    local player = AshitaCore:GetMemoryManager():GetPlayer();
    for _, id in ipairs(ids) do
        if state.keyitem_overrides[id] ~= nil then return true; end
        local _, authoritative = packet_has_keyitem(id);
        if authoritative then return true; end
        if player ~= nil and player:HasKeyItem(id) == true then return true; end
    end
    return false;
end
...
local function has_keyitem(name)
    ...
    local player = AshitaCore:GetMemoryManager():GetPlayer();
    for _, id in ipairs(ids) do
        -- v1.6.6 ownership priority:
        -- 1) live obtain/loss override, 2) authoritative 0x055 page,
        -- 3) Ashita memory, 4) saved cache for persistent Atmas / Abyssites only.
        -- CatsEye diagnostic confirmed Type=N maps to KI ids N*0x200..+0x1FF.
        local override = state.keyitem_overrides[id];
        if override ~= nil then
            if override == true then return true, id; end
            -- An explicit live loss must beat older packet/cache state.
        else
            local packet_owned, authoritative = packet_has_keyitem(id);
            if authoritative then
                if packet_owned then return true, id; end
            elseif player ~= nil and player:HasKeyItem(id) then
                remember_keyitem(id, true);
                return true, id;
            elseif is_persistent_keyitem_name(name) and persisted_has_keyitem(id) then
                return true, id;
            end
        end
    end
...
local function refresh_keyitems_from_live_memory()
    if not state.lookup_ready then rebuild_lookups(); end
    local player = AshitaCore:GetMemoryManager():GetPlayer();
    if player == nil then return false; end

    -- Re-read Ashita local memory without discarding packet-backed state.
    -- On servers that delay 0x055 until zone, chat-derived overrides below
    -- provide the immediate acquisition / loss update. This sends nothing.
    for _, entry in pairs(gather_watched_keyitems()) do
        local ids = resolve_keyitem_ids(entry.name) or {};
        for _, id in ipairs(ids) do
            -- Never let stale client memory overwrite a chat-derived change.
            -- The override remains in force until an authoritative 0x055 arrives.
            if state.keyitem_overrides[id] == nil then
                -- CatsEye can report false here for Abyssea KIs even when owned.
                -- Treat local memory as a positive-only fallback: it may add an
                -- owned KI, but it must never erase packet-backed persisted state.
                if player:HasKeyItem(id) == true then
                    remember_keyitem(id, true);
                end
            end
        end
    ...
    state.last_keyitem_memory_refresh = os.clock();
    return true;
end
...
local function refresh_current_ownership()
    if not state.lookup_ready then rebuild_lookups(); end
    scan_items(true);
    refresh_keyitems_from_live_memory();
end

local detect_keyitem_changes;

local function clean_chat_text(message)
    local text = tostring(message or '');
    local ok, stripped = pcall(function()
        return text:strip_colors():strip_translate(true);
    end);
    if ok and stripped ~= nil then text = stripped; end
    return text;
end

local function apply_keyitem_override(entry, owned)
    local ids = resolve_keyitem_ids(entry.name) or {};
    if #ids == 0 then return false; end
    for _, id in ipairs(ids) do
        state.keyitem_overrides[id] = owned == true;
        remember_keyitem(id, owned == true);
    end
    save_persisted_keyitems();
    return true;
end

local function find_watched_keyitem_in_text(text)
    local compact = normalize(text);
    if compact == '' then return nil; end
    local best = nil;
    local best_len = 0;
    for _, entry in pairs(gather_watched_keyitems()) do
        local needle = normalize(entry.name);
        if #needle > best_len and #needle >= 4 and compact:find(needle, 1, true) ~= nil then
            best = entry;
            best_len = #needle;
        end
    end
...
local function process_keyitem_chat(message)
    if message == nil then return; end
    local text = clean_chat_text(message);
    local compact = normalize(text);
    if compact == '' then return; end

    local action = nil;
    if compact:find('obtainedkeyitem', 1, true) or compact:find('obtainedakeyitem', 1, true) or
       compact:find('acquiredkeyitem', 1, true) then
        action = true;
    elseif compact:find('lostkeyitem', 1, true) or compact:find('lostakeyitem', 1, true) or
           compact:find('consumedkeyitem', 1, true) then
        action = false;
    end

    local entry = find_watched_keyitem_in_text(text);
    if action ~= nil and entry ~= nil then
        if apply_keyitem_override(entry, action) then
            state.pending_keyitem_chat_action = nil;
            state.pending_keyitem_chat_until = 0;
            -- Defer sounds / addon output outside text_in to avoid recursive chat.
            ashita.tasks.once(0.03, function()
                if not state.monitor_ready then return; end
                if state.zoning or os.clock() < (state.zone_alert_mute_until or 0) then
                    state.keyitem_snapshot[normalize(entry.name)] = keyitem_count(entry.name);
                else
    ...
    -- Some clients split "Obtained key item:" and the KI name across two chat
    -- lines. Remember the action briefly and match the following line by name.
    if action ~= nil then
        state.pending_keyitem_chat_action = action;
        state.pending_keyitem_chat_until = os.clock() + 2.0;
        return;
    end

    if state.pending_keyitem_chat_action ~= nil and os.clock() <= (state.pending_keyitem_chat_until or 0) then
        if entry ~= nil and apply_keyitem_override(entry, state.pending_keyitem_chat_action) then
            state.pending_keyitem_chat_action = nil;
            state.pending_keyitem_chat_until = 0;
            ashita.tasks.once(0.03, function()
                if not state.monitor_ready then return; end
    ...
    elseif os.clock() > (state.pending_keyitem_chat_until or 0) then
        state.pending_keyitem_chat_action = nil;
        state.pending_keyitem_chat_until = 0;
    end
end
...
detect_keyitem_changes = function()
...
local function parse_keyitem_log_packet(packet)
    ...
    for id = first_id, first_id + 0x1FF do
        ...
        -- Do not clear chat-derived overrides here. Live obtain/loss events are
        -- newer than the last packet snapshot and remain in force until replaced.
    end
...
local function render_blue_proc_group(window_key, current_key, group_id, compact)
    local labels = { piercing = 'PIERCING', slashing = 'SLASHING', blunt = 'BLUNT' };
    local ranges = { piercing = '06:00 - 13:59', slashing = '14:00 - 21:59', blunt = '22:00 - 05:59' };
    local active = window_key == current_key;

    if not active then
        imgui.PushStyleVar(ImGuiStyleVar_Alpha, 0.52);
    end

    if active then
        imgui.TextColored({ 0.38, 0.72, 1.0, 1.0 }, labels[window_key] .. '  -  CURRENT');
    else
        imgui.TextColored(colors.muted, labels[window_key]);
    end
    imgui.SameLine();
    imgui.TextColored(active and colors.normal or colors.muted, ranges[window_key]);

    for i, entry in ipairs(blue_procs[window_key]) do
        local ws, weapon = entry[1], entry[2];
        local pkey = window_key .. ':' .. ws;
        imgui.Checkbox(('##%s_blue_%s_%d'):format(group_id, window_key, i), checkbox_map(state.proc_blue, pkey));
        imgui.SameLine();
        imgui.TextColored(active and colors.normal or colors.muted, ws);
        imgui.SameLine(compact and 225 or 310);
        imgui.TextColored(active and colors.source or colors.muted, weapon);
    end
...
local function render_blue_procs()
    ...
    imgui.TextColored(colors.muted, 'All Blue proc weapon skills are shown. The current CatsEye Time window is highlighted.');
    ...
    local order = { 'piercing', 'slashing', 'blunt' };
    for idx, window_key in ipairs(order) do
        render_blue_proc_group(window_key, key, 'main', false);
        if idx < #order then
            imgui.Spacing();
            imgui.Separator();
            imgui.Spacing();
...
local function render_proc_tracker_window()
    ...
    imgui.PushStyleColor(ImGuiCol_WindowBg, { 0.012, 0.025, 0.042, persisted.tracker_bg_alpha });
    ...
    if imgui.Begin('Proc Tracker##abyssea_proc_tracker', state.proc_tracker_open) then
        if imgui.SetWindowFontScale ~= nil then
            imgui.SetWindowFontScale(persisted.tracker_font_scale);
        end
        imgui.TextColored(colors.title, 'PROC TRACKER');
        imgui.SameLine();
        ...
        if state.proc_track_blue[1] then
            ...
            for idx, window_key in ipairs(order) do
                imgui.Spacing();
                render_blue_proc_group(window_key, key, 'tracker', true);
                if idx < #order then imgui.Spacing(); end
            end
...
local function render_sound_settings()
    ...
    imgui.Spacing();
    imgui.Separator();
    imgui.TextColored(colors.header, 'TRACKER DISPLAY');
    ...
    imgui.TextColored(colors.normal, ('Background opacity: %d%%'):format(math.floor((persisted.tracker_bg_alpha * 100) + 0.5)));
    if imgui.SmallButton('-##tracker_alpha_down') then
        persisted.tracker_bg_alpha = clamp(persisted.tracker_bg_alpha - 0.05, 0.00, 1.00);
        save_display_settings();
    end
    imgui.SameLine();
    if imgui.SmallButton('+##tracker_alpha_up') then
        persisted.tracker_bg_alpha = clamp(persisted.tracker_bg_alpha + 0.05, 0.00, 1.00);
        save_display_settings();
    end
    imgui.SameLine();
    if imgui.SmallButton('Reset##tracker_alpha_reset') then
        persisted.tracker_bg_alpha = 0.90;
        save_display_settings();
    end
    imgui.TextColored(colors.muted, 'Applies to both the pinned NM Tracker and Proc Tracker. Text stays fully visible.');

    imgui.Spacing();
    imgui.TextColored(colors.normal, ('Tracker font size: %d%%'):format(math.floor((persisted.tracker_font_scale * 100) + 0.5)));
    if imgui.SmallButton('-##tracker_font_down') then
        persisted.tracker_font_scale = clamp(persisted.tracker_font_scale - 0.05, 0.75, 1.50);
        save_display_settings();
    end
    imgui.SameLine();
    if imgui.SmallButton('+##tracker_font_up') then
        persisted.tracker_font_scale = clamp(persisted.tracker_font_scale + 0.05, 0.75, 1.50);
        save_display_settings();
    end
    imgui.SameLine();
    if imgui.SmallButton('Reset##tracker_font_reset') then
        persisted.tracker_font_scale = 1.00;
        save_display_settings();
    end
    imgui.TextColored(colors.muted, '75% to 150%. Applies to both the pinned NM Tracker and Proc Tracker.');
    imgui.Spacing();
    imgui.Separator();
...
local function render_tracker_requirement(zone_index, req, depth, visited)
    ...
    visited = visited or {};
    local owned = req_owned(req);
    local verified = req.type ~= 'ki' or keyitem_verified_this_session(req.name);
    local c = owned and colors.owned or (verified and colors.missing or colors.muted);
    local indent = string.rep('   ', depth);
    local glyph = owned and '[+]' or (verified and '[-]' or '[?]');
...
local function render_tracker_window()
    ...
    imgui.PushStyleColor(ImGuiCol_WindowBg, { 0.025, 0.035, 0.055, persisted.tracker_bg_alpha });
...
local function render_requirement(req, zone_index)
    ...
    local verified = req.type ~= 'ki' or keyitem_verified_this_session(req.name);
    local status_color = owned and colors.owned or (verified and colors.missing or colors.muted);
    local label = (req.type == 'ki') and 'KEY ITEM' or 'POP ITEM';
    local status_text = owned and 'OWNED' or (verified and 'MISSING' or 'UNVERIFIED');
    ...
    if #list > 0 then
        ...
        if not nm.requirements or #nm.requirements == 0 then
            ...
            for _, req in ipairs(nm.requirements) do
                ...
                local c = owned and colors.owned or (verified and colors.missing or colors.muted);
                local status_text = owned and 'OWNED' or (verified and 'MISSING' or 'UNVERIFIED');
                imgui.TextColored(c, status_text);
                imgui.SameLine(72);
...
-- Event-driven ownership updates.
-- Key Item ownership is refreshed only from:
--   1) obtain/loss system text,
--   2) incoming 0x055 Key Item Log packets,
--   3) one silent startup/zone synchronization.
-- There is no recurring Key Item polling in d3d_present.
-- These handlers only react to data FFXI already sends to the client; the
-- addon never requests inventory or Key Item data from the server.
ashita.events.register('text_in', 'abyssea_status_text_in', function (e)
    ...
    process_keyitem_chat(e.message_modified or e.message);
end);
...
ashita.events.register('packet_in', 'abyssea_packet_in', function(e)
    ...
    if e.id == 0x000A then
        ...
        -- This is the only full KI ownership sync after zoning; normal KI changes
        -- are event-driven from system text / 0x055 after this point.
...
ashita.events.register('command', 'abyssea_command', function(e)
    ...
    elseif args[2]:lower() == 'refresh' then
        ...
        print('[Abyssea] Local ownership rescanned. KI changes are also tracked instantly from obtain/loss messages.');
...
ashita.events.register('d3d_present', 'abyssea_present', function()
    -- Do not render any addon UI on the title, login, or character-select screens.
    if not character_is_in_game() then return; end
