...
v1.7.5
- Added adjustable pinned NM tracker background opacity from 0% to 100%.
- Added adjustable pinned NM tracker font scale from 75% to 150%.
- Tracker text remains fully opaque when the background is made transparent.
- Display preferences are saved between sessions.

v1.7.6
- Pinned NM Tracker display opacity now also applies to the standalone Proc Tracker.
- Tracker font-size control now scales both the Pinned NM Tracker and Proc Tracker.
- Addon UI remains hidden on title/login/character-select screens and appears only after a character is in game.

v1.7.7
- Changed Key Item ownership tracking to event-driven updates.
- Removed the recurring 5-second Key Item memory scan from d3d_present.
- Key Items now update from obtain/loss system text and incoming 0x055 KI packets.
- Performs one silent KI synchronization at addon startup and after zoning.
- Physical item inventory updates remain event-driven from inventory packets.
- Keeps v1.7.6 tracker opacity/font-size controls and login-screen UI guard.
