...
ashita.events.register('d3d_present', 'abyssea_present', function()
    -- Lightweight local-memory fallback. Immediate KI acquisition / loss is
    -- handled from the client's own system text because some private servers
    -- do not send a fresh 0x055 until zoning. No server requests are sent.
    if not state.zoning and state.monitor_ready and os.clock() >= (state.zone_alert_mute_until or 0) then
        local now = os.clock();
        if (now - (state.last_keyitem_memory_refresh or 0)) >= 5.0 then
            if refresh_keyitems_from_live_memory() then
                detect_keyitem_changes();
            end
        end
    end
