...
local function resolve_keyitem_ids(name)
    ...
    -- Pop KIs are temporary key items and must resolve to their real Ashita KI
    -- id before Player:HasKeyItem can check them. Do an exact resource lookup on
    -- demand instead of depending on the lookup table having been populated at
    -- addon load time. This also covers KIs whose resources become available
    -- after login / zoning.
    local rm = AshitaCore:GetResourceManager();
    local matched = {};
    for id = 0, 65535 do
        local resource_name = rm:GetString('keyitems.names', id);
        if resource_name ~= nil and #resource_name > 1 and normalize(resource_name) == key then
            table.insert(matched, id);
        end
    end

    -- Only use the abbreviation matcher as a last resort. Never allow a fuzzy
    -- match to replace an exact KI match.
    if #matched == 0 then
...
local function has_keyitem(name)
    ...
    -- Use the same ownership test as Ashita v4 ItemWatch. This works for both
    -- permanent KIs (Atmas / Abyssites) and temporary NM pop KIs. When Player
    -- memory is available it is authoritative in both directions, so consumed
    -- pop KIs immediately return to MISSING instead of being held by a stale
    -- persisted or packet cache.
    ...
    if player ~= nil then
        ...
        for _, id in ipairs(ids) do remember_keyitem(id, false); end
        return false, ids[1];
    end

    -- Startup-only fallback while Player memory is unavailable.
    for _, id in ipairs(ids) do
...
local function keyitem_count(name)
    ...
    for _, id in ipairs(ids) do
        local owned = false;
        if player ~= nil then
            owned = player:HasKeyItem(id) == true;
            remember_keyitem(id, owned);
        else
